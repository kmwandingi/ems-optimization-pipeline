    ...
    def optimize_centralized(self):
        ...
        for day_val in sorted(all_days):
            ...
            for hour in range(n_hours):
                hour_load_list = []
                fixed_load_for_hour = 0
                
                # Add loads from all devices
                for dev, (d_idx, consumption_24, prices_24) in device_indices.items():
                    if dev.is_flexible:
                        # Handle flexible devices with shift variables
                        for t in range(n_hours):
                        ...
                        # For non-flexible devices, directly add their fixed load to this hour
                        # This is critical - non-flexible device loads must still be accounted for
                        if hour < len(consumption_24):
                            fixed_load_for_hour += consumption_24[hour]
                
                # Add building load constraint integrating non-flexible loads, battery and EV if available
                if hour_load_list or fixed_load_for_hour > 0 or battery_state is not None or ev_state is not None:  # Only add constraint if there are components to consider
                    load_sum = lpSum(hour_load_list) if hour_load_list else 0
                    # Add the fixed load from non-flexible devices
                    load_sum += fixed_load_for_hour
