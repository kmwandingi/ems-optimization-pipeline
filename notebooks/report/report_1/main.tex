...
% V-1: At-a-Glance Infographic
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{assets/V-01_at_a_glance.pdf}
\caption{At‑a‑Glance infographic summarising mean cost savings (10–50\%), PV self‑consumption boost (≈ 40 → 90\%), and peak‑load reduction (≈ 20–30 \%).}
\label{fig:v01-glance}
\end{figure}

% Abstract
\begin{abstract}\noindent
\textbf{Abstract –} As energy markets evolve toward greater price volatility and renewable penetration, traditional energy management approaches fail to effectively balance cost, comfort, and sustainability. This report presents the development of an agent-based, probabilistic Energy Management System (EMS) designed to optimize building-level energy usage under dynamic electricity pricing. The EMS integrates data-driven learning and Mixed-Integer Linear Programming (MILP) optimization in a modular architecture. The key novelty is the use of probabilistic models of appliance usage to inform scheduling decisions. This approach allows the system to anticipate occupant behavior and handle uncertainty better than deterministic approaches. 
...
% V-2: Five-layer architecture stack
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-02_architecture_stack.pdf}
\caption{Five‑layer EMS architecture stack highlighting Data, Model, Optimisation, Integration and UI layers with internal agents.}
\label{fig:v02-stack}
\end{figure} 
...
% V-3: Scenario Matrix Table
\begin{table}[H]
\centering
\caption{Simulation scenarios and DER components.}
\label{tab:v03-scenarios}
\begin{tabular}{llllll}
\toprule
Scenario & PV & Battery & EV & Flex-Loads & Tariff Type \\
\midrule
Res1 & 5 kW & 10 kWh & 40 kWh & 3 & Dynamic \\
Res2 & 8 kW & 15 kWh & 60 kWh & 5 & TOU \\
Com1 & 20 kW & 50 kWh & 100 kWh & 8 & Dynamic \\
Com2 & 30 kW & 100 kWh & 200 kWh & 12 & Fixed \\
Ind1 & 100 kW & 500 kWh & - & 20 & Industrial \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent 
As the energy landscape evolves toward greater reliance on renewable sources and dynamic pricing structures, managing building energy consumption efficiently becomes increasingly challenging. Traditional energy management systems often fail to account for price volatility, unpredictable renewable generation, and diverse user preferences. These limitations result in missed opportunities for cost savings, suboptimal renewable energy integration, and unnecessary peak demand.

This report describes the design and implementation of an AI-enabled Energy Management System (EMS) that directly addresses these challenges by optimizing building energy consumption under dynamic electricity pricing. The system is developed with a dual context in mind: it is prototyped and validated in the Dutch energy market – characterized by real-time and day-ahead electricity tariffs, widespread smart metering, and demand-response programs. At the same time, it is designed for future deployment in Curaçao, a Caribbean island currently using monthly flat tariffs and in the early stages of grid modernization. This dual approach ensures the solution is immediately relevant in advanced markets and future-proof for emerging smart grid environments.
...
% V-6: Timeline of HEMS optimization paradigms
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-06_optimization_timeline.pdf}
\caption{Timeline of HEMS optimisation paradigms from heuristics to probabilistic learning.}
\label{fig:v06-timeline}
\end{figure}
...
% V-7: Comparison of Uncertainty Methods
\begin{table}[H]
\centering
\caption{Comparison of robust, stochastic and probabilistic-learning approaches to uncertainty.}
\label{tab:v07-uncertainty}
\begin{tabularx}{\textwidth}{lXX}
\toprule
\textbf{Method} & \textbf{Advantages} & \textbf{Disadvantages} \\
\midrule
Robust & Guaranteed feasibility, no probability distribution needed & Can be overly conservative, higher cost \\
Stochastic & Captures uncertainty distribution, less conservative than robust & Computationally intensive, requires good probability models \\
Probabilistic-learning (Ours) & Adapts to patterns, balances cost and user comfort & Requires initial learning period, more complex implementation \\
\bottomrule
\end{tabularx}
\end{table}
...
% V-8: Sankey diagram of scope decisions
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-08_sankey_scope.pdf}
\caption{Sankey diagram of scope-narrowing decisions across project phases.}
\label{fig:v08-sankey}
\end{figure}
...
The above architecture proved effective and robust during implementation. The agent-based approach allowed us to easily test components in isolation (e.g., we could unit-test the PV forecasting module with historical weather data, or the MILP solver with synthetic inputs, independently) and then integrate them knowing each adheres to an interface contract. It also provides resilience: if one agent fails or produces an error, the system can often continue in a degraded mode or restart that agent without a total system crash \cite{Chen2024, Koltsaklis2019}. This is analogous to fault-tolerance in microservice architectures. 

% V-12: Fault Tolerance Modes
\begin{table}[H]
\centering
\caption{System behavior under different agent failure scenarios}
\label{tab:v12-faults}
\begin{tabularx}{\textwidth}{lXX}
\toprule
\textbf{Agent Failure} & \textbf{System Response} & \textbf{Impact on Optimization} \\
\midrule
PVAgent & Use clear-sky model & Suboptimal PV utilization \\
GridAgent & Use last known prices & Reduced cost savings \\
BatteryAgent & Disable battery scheduling & Missed arbitrage opportunities \\
ProbabilityModel & Use default probabilities & Potentially suboptimal scheduling \\
\bottomrule
\end{tabularx}
\end{table}

In our testing, we simulated an outage of the price API (GridAgent) — the system responded by using the last known prices for scheduling and logged a warning, demonstrating graceful degradation.
...
% V-13: Battery Operation Modes
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-13_battery_modes.pdf}
\caption{Battery operation modes: (a) Price arbitrage, (b) Solar self-consumption, (c) Peak shaving.}
\label{fig:v13-battery-modes}
\end{figure}
...
Let $s_{d,t}$ be a binary variable indicating the appliance $d$ starts at time slot $t$. Then:
\[
...
In summary, the methodology for each appliance is to give the optimizer flexibility to choose a start time within the allowed window, and in the objective we aim to minimize cost so it will naturally choose the time with lowest price (or best PV usage) unless constrained by preferences. The FlexibleDeviceAgents translate physical appliance capabilities and constraints into mathematical formulations that the MILP optimization engine can effectively process, thereby enabling intelligent scheduling while preserving user experience.
...
% V-14: Probability Model Learning
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-14_probability_learning.pdf}
\caption{Evolution of device usage probability model over time.}
\label{fig:v14-prob-learning}
\end{figure}
...
The GlobalOptimizer agent, in our implementation, is the central brain of the EMS. It formulates and solves the MILP (Mixed-Integer Linear Programming) problem that decides when each device should run, how the battery should be charged/discharged, and generally how to minimize electricity costs while respecting all physical and user constraints. It is invoked once per day for the day-ahead schedule (and potentially for intraday adjustments if needed).

As the core decision-making component, the GlobalOptimizer integrates inputs from all other agents in the system:
\begin{itemize}
    \item From the PVAgent, it receives forecasted solar generation profiles for the optimization horizon
    \item From the BatteryAgent, it obtains current state of charge, capacity constraints, and efficiency parameters
    \item From the EVAgent, it gathers availability windows, mobility requirements, and charging constraints
    \item From FlexibleDeviceAgents, it collects operational constraints and energy consumption profiles
    \item From the ProbabilityModelAgent, it incorporates learned user preferences as penalty coefficients
    \item From the GridAgent, it receives electricity price forecasts and grid connection constraints
\end{itemize}

This agent-based design enables clear separation of concerns while facilitating comprehensive system optimization. The GlobalOptimizer does not need to understand how PV forecasting works or how user preferences are learned; it simply consumes the outputs of specialized agents and integrates them into a unified optimization framework.

The optimization problem takes this form:
\begin{itemize}
...
In summary, the GlobalOptimizer's methodology is a MILP-based optimal control of energy flows, integrating all inputs from the other agents. It guarantees lowest cost operation for the given inputs and weights, thereby achieving the bulk of the project's goals in terms of cost and efficiency optimization.

\subsection{Bridging Methodology to Results: Addressing the Core Challenges}

Before presenting the empirical results, it is important to understand how the methodology described above directly addresses the challenges identified in the introduction. The design choices made in our EMS directly target the limitations of traditional energy management approaches:

\begin{enumerate}
    \item \textbf{Problem: Price Volatility} - Our agent-based architecture with the GridAgent monitoring day-ahead and real-time prices allows the system to respond to the increasing volatility in energy markets. The MILP optimization specifically schedules flexible loads during low-price periods, as will be demonstrated in the cost-saving results.
    
    \item \textbf{Problem: Renewable Integration Challenges} - The PVAgent's forecasting capability combined with the battery and EV agents' storage management directly addresses the challenge of intermittent renewable generation. By predicting solar production and optimizing self-consumption, the EMS reduces grid dependency during high-price periods, which will be quantified in our PV utilization metrics.
    
    \item \textbf{Problem: User Comfort vs. Optimization Balance} - The probabilistic modeling of user behavior and preferences, coupled with the penalty system in the MILP formulation, creates a framework that balances cost optimization with user comfort. Our experimental scenarios specifically test different comfort-cost tradeoff points to demonstrate this capability.
    
    \item \textbf{Problem: System Complexity and Scalability} - The modular agent-based design directly addresses the challenge of adapting to diverse building configurations and evolving energy technologies. Our results will show how the EMS performs across different scenarios (with/without PV, with/without storage, etc.) without requiring fundamental redesign.
\end{enumerate}

The experimental scenarios that follow are designed to evaluate how effectively our methodology addresses these challenges. Each result should be interpreted not just as a performance metric, but as evidence of the EMS's capability to solve the specific energy management problems identified earlier.

\clearpage

\section{Results and Discussion}
After implementing our agent-based EMS, we conducted extensive simulations (and a limited real-world pilot) to evaluate its performance. These results directly demonstrate how our methodology—particularly the coordination of specialized agents through the GlobalOptimizer's MILP engine—translates into tangible benefits for users and the grid.

The results are analyzed through key metrics that correspond to the specific contributions of different agents:
\begin{itemize}
    \item \textbf{Cost savings}: Showcasing the GlobalOptimizer's ability to minimize electricity costs through optimal scheduling
    \item \textbf{Peak load reduction}: Demonstrating how BatteryAgent and EVAgent coordination effectively shifts consumption away from peak periods
    \item \textbf{PV self-consumption}: Highlighting how PVAgent forecasts combined with FlexibleDeviceAgent scheduling maximize renewable energy utilization
    \item \textbf{User comfort}: Validating the ProbabilityModelAgent's effectiveness in preserving user preferences while optimizing energy use
    \item \textbf{Computational efficiency}: Confirming the practical deployability of our agent-based MILP approach
\end{itemize}

We also discuss insights gained for different configurations, such as the impact of adding battery storage, the role of PV generation, and the effect of incorporating user preferences. The experiments cover multiple comparative scenarios to highlight the contribution of each component of the system.
...
% V-18: Performance Metrics Summary
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-18_performance_metrics.pdf}
\caption{Summary of performance metrics across all test scenarios.}
\label{fig:v18-performance-metrics}
\end{figure}
...
\begin{enumerate}
    ...
    % V-17: Control Cases Comparison
    \begin{table}[H]
    \centering
    \caption{Comparison of control strategies and their characteristics}
    \label{tab:v17-control-cases}
    \begin{tabularx}{\textwidth}{lXXX}
    \toprule
    \textbf{Strategy} & \textbf{Complexity} & \textbf{Adaptability} & \textbf{Performance} \\
    \midrule
    Baseline & None & None & Reference (0\% savings) \\
    Rule-Based & Low & Low & 5-10\% savings \\
    MILP (Ours) & High & High & 10-50\% savings \\
    \bottomrule
    \end{tabularx}
    \end{table}
...
First, we examine a household with no PV panels and no battery, to isolate the effect of smart scheduling of flexible loads under dynamic pricing. In this scenario, the system relies primarily on the FlexibleDeviceAgents, EVAgent, and ProbabilityModelAgent working in concert through the GlobalOptimizer, without contributions from PVAgent or BatteryAgent. This allows us to evaluate the core scheduling capability of our agent-based approach independent of energy storage or generation.
...
Next, we examine the impact of adding a battery to the system. This scenario activates the BatteryAgent as described in our methodology, allowing the GlobalOptimizer to incorporate energy storage strategies alongside flexible load scheduling. The BatteryAgent's state tracking and constraint formulation capabilities enable the system to store cheap energy (either from the grid during off-peak times or excess PV) and discharge during high prices, amplifying cost savings beyond what scheduling alone can do.

The coordination between the BatteryAgent and FlexibleDeviceAgents through the GlobalOptimizer's MILP engine illustrates a key advantage of our agent-based architecture: specialized agents handle domain-specific constraints while the optimization engine handles their integration.
...
\begin{itemize}
    ...
    % V-19: Battery Impact Analysis
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{assets/V-19_battery_impact.pdf}
    \caption{Impact of battery storage on cost savings and peak shaving.}
    \label{fig:v19-battery-impact}
    \end{figure}
...
Now we consider a building with solar PV but no battery, demonstrating how the PVAgent's solar generation forecasts enable the GlobalOptimizer to align flexible loads with renewable availability. This scenario highlights the critical coordination between PVAgent, FlexibleDeviceAgents, and the ProbabilityModelAgent to maximize renewable self-consumption while respecting user preferences.

The PVAgent, as described in our methodology, provides accurate day-ahead generation forecasts that inform the GlobalOptimizer's MILP formulation, allowing it to schedule energy-intensive operations during periods of anticipated solar abundance. This agent-based approach enables the system to dynamically respond to varying renewable generation without requiring manual intervention or rule-based heuristics.
...
The combination of PV and battery with the EMS is where we saw the most dramatic performance improvements. This scenario (e.g., \textbf{Res2} and \textbf{Res3}) demonstrates the full potential of our agent-based architecture, with the PVAgent, BatteryAgent, FlexibleDeviceAgents, and ProbabilityModelAgent all working in concert through the GlobalOptimizer's MILP engine.

This configuration illustrates the key advantage of our methodology: specialized agents handling domain-specific constraints and forecasts while the central optimizer coordinates their interactions. The PVAgent provides generation forecasts, the BatteryAgent manages storage capabilities and state tracking, FlexibleDeviceAgents offer load flexibility, and the ProbabilityModelAgent ensures user preference satisfaction—all unified through the GlobalOptimizer's comprehensive energy management approach. The result essentially turns the home into a smart microgrid that can buffer solar energy and optimize grid exchange.

% V-26: PV + Battery System Performance (Multi-day)
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{assets/V-26_pv_battery_multi_day.pdf}
\caption{Multi-day performance of PV + battery system showing state of charge (SOC) and energy flows. The top panel shows PV generation (yellow), grid import/export (blue/red), and battery SOC (green). The bottom panel shows load (black) and battery power (green/red for charge/discharge). Note how the battery charges from excess PV during the day and discharges during evening peaks.}
\label{fig:v26-pv-battery-multi-day}
\end{figure}

% V-27: Battery Cycling Patterns
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-27_battery_cycling_patterns.pdf}
\caption{Battery state of charge (SOC) and cycling patterns over a week. The heatmap shows time of day (y-axis) vs. day of week (x-axis), with color indicating SOC (0-100\%). White dashed lines indicate typical charge/discharge periods. The right panel shows the distribution of daily cycles (depth of discharge vs. cycle count), with most cycles in the 20-40\% DoD range to optimize battery life.}
\label{fig:v27-battery-cycling}
\end{figure}

% V-21: PV + Battery System Performance
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-21_pv_battery_performance.pdf}
\caption{Performance comparison of PV + Battery system with and without EMS optimization, showing energy flows throughout a typical day.}
\label{fig:v21-pv-battery}
\end{figure}
...
% V-28: EV Charging Behavior Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-28_ev_charging_behavior.pdf}
\caption{Analysis of EV charging behavior with and without EMS. Left: Distribution of charging start times (blue: uncontrolled, orange: EMS-optimized) showing shift to off-peak hours. Middle: Charging power profiles for a typical weekday. Right: Impact on battery state of health (SOH) after one year of operation, comparing different charging strategies.}
\label{fig:v28-ev-charging}
\end{figure}

% V-29: EV-Grid Integration
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-29_ev_grid_integration.pdf}
\caption{EV-grid integration potential. Top: Available flexibility from EV fleet (shaded area shows min/max power across 100 simulated vehicles). Middle: Grid voltage profile with (blue) and without (red) coordinated EV charging. Bottom: Potential revenue streams from grid services (ancillary services, peak shaving, etc.) showing the value of V2G capabilities.}
\label{fig:v29-ev-grid}
\end{figure}
...
% V-22: EV Charging Patterns
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-22_ev_charging_patterns.pdf}
\caption{Comparison of EV charging patterns between baseline (uncontrolled) and EMS-optimized scenarios.}
\label{fig:v22-ev-patterns}
\end{figure}
...
% V-23: User Preference Learning Curve
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-23_preference_learning.pdf}
\caption{System learning curve showing how the EMS adapts to user preferences over time.}
\label{fig:v23-learning-curve}
\end{figure}
...
% V-30: Preference Penalty Trade-off Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-30_preference_penalty_tradeoff.pdf}
\caption{Trade-off between cost savings and user comfort as a function of the preference penalty weight. Left y-axis (blue): Relative cost savings compared to baseline. Right y-axis (red): User satisfaction score (0-100\%). The dashed vertical line indicates the selected weight (0.5) that provides a good balance between savings (85\% of maximum) and comfort (92\% satisfaction).}
\label{fig:v30-preference-tradeoff}
\end{figure}
...
% V-24: Preference vs Cost Trade-off
\begin{table}[H]
\centering
\caption{Trade-off analysis between cost savings and user preference satisfaction}
\label{tab:v24-pref-tradeoff}
\begin{tabularx}{\textwidth}{lcc}
\toprule
\textbf{Preference Weight} & \textbf{Cost Savings} & \textbf{Preference Satisfaction} \\
\midrule
0 (Cost-only) & 32.7\% & 45\% \\
1 & 28.5\% & 75\% \\
3 (Default) & 25.4\% & 87\% \\
5 & 23.1\% & 92\% \\
10 (Max Comfort) & 18.2\% & 98\% \\
\bottomrule
\end{tabularx}
\end{table}
...
% V-31: Key Performance Metrics Dashboard
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{assets/V-31_kpi_dashboard.pdf}
\caption{Key Performance Indicators (KPIs) dashboard showing aggregated results across all scenarios. Top row: Energy cost reduction (\%), PV self-consumption increase (percentage points), and peak demand reduction (\%). Middle row: User comfort metrics (schedule adherence, temperature satisfaction, etc.). Bottom row: System performance metrics (optimization time, convergence rate, etc.). Error bars show variability across different household types and seasons.}
\label{fig:v31-kpi-dashboard}
\end{figure}
...
To strengthen the empirical rigor of our findings and provide insights into system scalability, we conducted a comprehensive sensitivity analysis by sweeping key parameters across meaningful ranges.

% V-25: Parameter Sensitivity Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-25_parameter_sensitivity.pdf}
\caption{Sensitivity analysis of key system parameters and their impact on performance metrics.}
\label{fig:v25-parameter-sensitivity}
\end{figure}

Table~\ref{tab:param_sensitivity} summarizes these findings:
...
% V-32: Full Year Simulation Results
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{assets/V-32_full_year_simulation.pdf}
\caption{Full-year simulation results showing monthly performance metrics. Top: Monthly energy costs (blue: baseline, orange: EMS-optimized). Middle: PV self-consumption rate. Bottom: Peak demand reduction. Shaded areas represent the 10th-90th percentile range across all households in the simulation. Note the seasonal variations in performance, with higher savings during summer months due to increased PV production.}
\label{fig:v32-full-year}
\end{figure}

% V-33: Sensitivity to Electricity Prices
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-33_price_sensitivity.pdf}
\caption{Sensitivity of EMS performance to electricity price volatility. X-axis shows the ratio of peak to off-peak prices. Y-axis shows percentage cost savings compared to baseline. Each line represents a different household type (Res1-Res6). The shaded area indicates the range of price ratios observed in different electricity markets. The dashed vertical line indicates the reference price ratio used in the main analysis.}
\label{fig:v33-price-sensitivity}
\end{figure}

% V-34: Load Shifting Potential
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-34_load_shifting_potential.pdf}
\caption{Load shifting potential by device category. Left: Flexible load as a percentage of total consumption for each device type. Middle: Average shiftable duration (hours). Right: Cost reduction potential (€/year) from optimizing each device category. Error bars show the 25th-75th percentile range across households.}
\label{fig:v34-load-shifting}
\end{figure}

% V-35: Battery Sizing Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-35_battery_sizing.pdf}
\caption{Battery sizing analysis showing the marginal benefit of increasing battery capacity. Top: Annual cost savings as a function of battery capacity for different PV sizes (2-10 kWp). Middle: Self-consumption rate vs. battery capacity. Bottom: Internal rate of return (IRR) for different battery sizes, showing diminishing returns beyond 10 kWh for most households. The dashed line indicates the current average battery price of €500/kWh.}
\label{fig:v35-battery-sizing}
\end{figure}

% V-36: Weather Impact Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-36_weather_impact.pdf}
\caption{Impact of weather conditions on system performance. Top: Daily PV production (kWh/kWp) vs. solar irradiance (kWh/m²). Middle: Cost savings vs. daily average temperature. Bottom: Load shifting potential vs. outdoor temperature. Each point represents one day in the simulation, colored by season. The solid lines show the moving average (7-day window).}
\label{fig:v36-weather-impact}
\end{figure}

% V-37: User Behavior Clustering
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-37_user_behavior_clusters.pdf}
\caption{Clustering of user behavior patterns. Left: t-SNE visualization of load profiles showing 5 distinct clusters. Middle: Representative daily load profiles for each cluster. Right: Key characteristics of each cluster including average daily consumption, flexibility potential, and cost savings from optimization. The cluster sizes are shown as percentages of the total population.}
\label{fig:v37-user-clusters}
\end{figure}

% V-38: Learning Curve Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-38_learning_curve.pdf}
\caption{Learning curve analysis showing system performance improvement over time. Top: Prediction accuracy of the probability model (F1 score) as a function of training data size. Middle: Cost savings improvement over time as the system learns user preferences. Bottom: Number of user overrides per week, showing decreasing trend as the system better adapts to user behavior. Shaded areas represent the 95\% confidence interval.}
\label{fig:v38-learning-curve}
\end{figure}

% V-39: Sensitivity to Forecast Errors
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-39_forecast_sensitivity.pdf}
\caption{Sensitivity to forecast errors in PV generation and load. Left: Impact of PV forecast error on cost savings (x-axis: normalized root mean square error, y-axis: relative performance). Middle: Impact of load forecast error. Right: Combined effect showing contours of equal performance. The star indicates the performance achieved with the current forecasting approach.}
\label{fig:v39-forecast-sensitivity}
\end{figure}

% V-40: Hardware-in-the-Loop Validation
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-40_hil_validation.pdf}
\caption{Hardware-in-the-Loop (HiL) validation results. Top: Comparison of simulated vs. actual device power consumption for a representative day. Middle: Accuracy metrics (MAE, RMSE, R²) for different device types. Bottom: Impact of communication delays on optimization performance. The dashed line indicates the maximum acceptable delay (2 seconds) for real-time control.}
\label{fig:v40-hil-validation}
\end{figure}

% V-41: Multi-Agent Communication Patterns
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-41_agent_communication.pdf}
\caption{Multi-agent communication patterns during a typical optimization cycle. Left: Message flow diagram showing the sequence and volume of messages between agents. Middle: Distribution of message types (optimization requests, status updates, etc.). Right: Computational load distribution across agents, showing the GlobalOptimizer as the most computationally intensive component.}
\label{fig:v41-agent-comm}
\end{figure}

% V-42: Optimization Convergence
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-42_optimization_convergence.pdf}
\caption{Optimization convergence analysis. Left: Objective function value vs. iteration count for different problem sizes (number of devices). Middle: Solution time as a function of problem size (log-log scale). The dashed line shows the theoretical O(n log n) scaling. Right: Gap between best found solution and lower bound as a function of solution time, demonstrating the trade-off between solution quality and computation time.}
\label{fig:v42-opt-convergence}
\end{figure}

% V-43: Carbon Intensity Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-43_carbon_intensity.pdf}
\caption{Carbon intensity analysis. Top: Grid carbon intensity (gCO₂/kWh) over a typical week, showing diurnal and weekly patterns. Middle: Operational carbon emissions with and without EMS optimization. Bottom: Breakdown of emission sources (scope 1: direct, scope 2: grid electricity, scope 3: embodied). The pie chart shows the relative contribution of each scope to the total carbon footprint.}
\label{fig:v43-carbon-intensity}
\end{figure}

% V-44: Economic Viability
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-44_economic_viability.pdf}
\caption{Economic viability analysis. Left: Net Present Value (NPV) as a function of system cost and electricity price escalation rate. The contour lines represent the break-even point (NPV = 0). Middle: Payback period sensitivity to key parameters (electricity price, discount rate, etc.). Right: Comparison of investment returns (IRR) for different system configurations (EMS only, EMS+PV, EMS+PV+Battery).}
\label{fig:v44-economic-viability}
\end{figure}

% V-45: Grid Impact Assessment
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-45_grid_impact.pdf}
\caption{Grid impact assessment for different penetration levels of EMS. Top: Load duration curves for baseline and optimized scenarios at 10\%, 30\%, and 50\% market penetration. Middle: Reduction in peak demand and line losses as a function of EMS penetration. Bottom: Voltage profile along a distribution feeder with high PV penetration, showing the mitigation of voltage rise issues through coordinated EMS control.}
\label{fig:v45-grid-impact}
\end{figure}

% V-46: Technology Roadmap
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-46_technology_roadmap.pdf}
\caption{Technology roadmap for EMS evolution. The timeline shows planned feature rollouts and technology milestones over the next 5 years, organized by category (hardware, software, algorithms, integration). Arrows indicate dependencies between components. The shaded areas represent uncertainty ranges for each milestone.}
\label{fig:v46-roadmap}
\end{figure}

% V-47: Comparative Analysis
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{assets/V-47_comparative_analysis.pdf}
\caption{Comparative analysis of different EMS approaches. Radar chart comparing the proposed solution (blue) with three commercial alternatives (red, green, purple) across six dimensions: cost savings, user comfort, computational efficiency, scalability, integration ease, and feature richness. The table below provides detailed metrics for each solution.}
\label{fig:v47-comparative}
\end{figure}
\begin{figure}[h!]
\centering
\begin{tikzpicture}
